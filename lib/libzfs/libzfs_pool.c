...
shared_log_cb(zpool_handle_t *hdl, void *arg)
{
	...
	if (fnvlist_lookup_uint64(hdl->zpool_config, ZPOOL_CONFIG_POOL_GUID) == data->guid) {
...
{
	uint64_t guid;
	if (nvlist_lookup_uint64(zhp->zpool_config, ZPOOL_CONFIG_SHARED_LOG_POOL, &guid) != 0) {
		return (NULL);
	}
...
zpool_destroy(zpool_handle_t *zhp, const char *log_str)
{
	...
	if (err != 0) {
		...
		if (err == EROFS) {
			...
			while ((elem = nvlist_next_nvpair(clients, elem)) != NULL && idx < ERRBUFLEN) {
				idx += snprintf(buf + idx, ERRBUFLEN - idx, ", %s", nvpair_name(elem));
			}
    ...
    const char *log_str)
{
	...
	nvlist_t *outnvl;
	int err = lzc_pool_export(zhp->zpool_name, log_str, force, hardforce, &outnvl);
	if (err == ZFS_ERR_IOC_CMD_UNAVAIL) {
