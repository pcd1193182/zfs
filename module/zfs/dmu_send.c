 ...
 */
static int
dump_record(dmu_sendarg_t *dsa, void *payload, int payload_len)
{
	dmu_send_outparams_t *dso = dsa->dsa_dso;
	ASSERT3U(offsetof(dmu_replay_record_t, drr_u.drr_checksum.drr_checksum),
	    ==, sizeof (dmu_replay_record_t) - sizeof (zio_cksum_t));
	(void) fletcher_4_incremental_native(dsa->dsa_drr,
	    offsetof(dmu_replay_record_t, drr_u.drr_checksum.drr_checksum),
	    &dsa->dsa_zc);
	if (dsa->dsa_drr->drr_type == DRR_BEGIN) {
		dsa->dsa_sent_begin = B_TRUE;
	} else {
		ASSERT(ZIO_CHECKSUM_IS_ZERO(&dsa->dsa_drr->drr_u.
		    drr_checksum.drr_checksum));
		dsa->dsa_drr->drr_u.drr_checksum.drr_checksum = dsa->dsa_zc;
	}
	if (dsa->dsa_drr->drr_type == DRR_END) {
		dsa->dsa_sent_end = B_TRUE;
	}
	(void) fletcher_4_incremental_native(&dsa->dsa_drr->
	    drr_u.drr_checksum.drr_checksum,
	    sizeof (zio_cksum_t), &dsa->dsa_zc);
	*dsa->dsa_off += sizeof (dmu_replay_record_t);
	dsa->dsa_err = dso->dso_outfunc(dsa->dsa_os, dsa->dsa_drr,
	    sizeof (dmu_replay_record_t), dso->dso_arg);
	if (dsa->dsa_err != 0)
		return (SET_ERROR(EINTR));
	if (payload_len != 0) {
		*dsa->dsa_off += payload_len;
		/*
		...
		if (payload != NULL) {
			(void) fletcher_4_incremental_native(
			    payload, payload_len, &dsa->dsa_zc);
		}
		...
		dsa->dsa_err = dso->dso_outfunc(dsa->dsa_os, payload,
 ...
 */
static int
dump_free(dmu_sendarg_t *dsa, uint64_t object, uint64_t offset,
    uint64_t length)
{
	struct drr_free *drrf = &(dsa->dsa_drr->drr_u.drr_free);
	 ...
	 * satisfied.
	 */
	ASSERT(object > dsa->dsa_last_data_object ||
	    (object == dsa->dsa_last_data_object &&
	    offset > dsa->dsa_last_data_offset));
	 ...
	 */
	if (dsa->dsa_pending_op != PENDING_NONE &&
	    dsa->dsa_pending_op != PENDING_FREE) {
		if (dump_record(dsa, NULL, 0) != 0)
			return (SET_ERROR(EINTR));
		dsa->dsa_pending_op = PENDING_NONE;
	}

	if (dsa->dsa_pending_op == PENDING_FREE) {
		/*
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_FREE;
	...
	if (offset + length < offset)
		...
		drrf->drr_length = length;
	drrf->drr_toguid = dsa->dsa_toguid;
...
static int
dmu_dump_write(dmu_sendarg_t *dsa, dmu_object_type_t type, uint64_t object,
    uint64_t offset, int lsize, int psize, const blkptr_t *bp,
    void *data)
{
	uint64_t payload_size;
	boolean_t raw = (dsa->dsa_featureflags & DMU_BACKUP_FEATURE_RAW);
	struct drr_write *drrw = &(dsa->dsa_drr->drr_u.drr_write);
	...
	ASSERT(object > dsa->dsa_last_data_object ||
	    (object == dsa->dsa_last_data_object &&
	    offset > dsa->dsa_last_data_offset));
	dsa->dsa_last_data_object = object;
	dsa->dsa_last_data_offset = offset + lsize - 1;
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_WRITE;
	drrw->drr_object = object;
	drrw->drr_type = type;
	drrw->drr_offset = offset;
	drrw->drr_toguid = dsa->dsa_toguid;
	drrw->drr_logical_size = lsize;

	/* only set the compression fields if the buf is compressed or raw */
	if (raw || lsize != psize) {
		ASSERT(bp != NULL);
		ASSERT(raw || dsa->dsa_featureflags &
	...
	if (dump_record(dsa, data, payload_size) != 0)
		return (SET_ERROR(EINTR));
	return (0);
}

static int
dump_write_embedded(dmu_sendarg_t *dsa, uint64_t object, uint64_t offset,
    int blksz, const blkptr_t *bp)
{
	char buf[BPE_PAYLOAD_SIZE];
	struct drr_write_embedded *drrw =
	    &(dsa->dsa_drr->drr_u.drr_write_embedded);
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_WRITE_EMBEDDED;
	drrw->drr_object = object;
	drrw->drr_offset = offset;
	...
	if (dump_record(dsa, buf, rsize) != 0)
		return (SET_ERROR(EINTR));
	return (0);
}

static int
dump_spill(dmu_sendarg_t *dsa, const blkptr_t *bp, uint64_t object,
    void *data)
{
	struct drr_spill *drrs = &(dsa->dsa_drr->drr_u.drr_spill);
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_SPILL;
	drrs->drr_object = object;
	drrs->drr_length = blksz;
	drrs->drr_toguid = dsa->dsa_toguid;

	/* See comment in piggyback_unmodified_spill() for full details */
	if (zfs_send_unmodified_spill_blocks &&
	    (BP_GET_LOGICAL_BIRTH(bp) <= dsa->dsa_fromtxg)) {
...
static int
dump_freeobjects(dmu_sendarg_t *dsa, uint64_t firstobj, uint64_t numobjs)
{
	struct drr_freeobjects *drrfo = &(dsa->dsa_drr->drr_u.drr_freeobjects);
	uint64_t maxobj = DNODES_PER_BLOCK *
	    (DMU_META_DNODE(dsa->dsa_os)->dn_maxblkid + 1);
	 ...
	 */
	if (dsa->dsa_pending_op != PENDING_NONE &&
	    dsa->dsa_pending_op != PENDING_FREEOBJECTS) {
		if (dump_record(dsa, NULL, 0) != 0)
			return (SET_ERROR(EINTR));
		dsa->dsa_pending_op = PENDING_NONE;
	}

	if (dsa->dsa_pending_op == PENDING_FREEOBJECTS) {
		/*
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_FREEOBJECTS;
	drrfo->drr_firstobj = firstobj;
	drrfo->drr_numobjs = numobjs;
	drrfo->drr_toguid = dsa->dsa_toguid;
...
static int
dump_dnode(dmu_sendarg_t *dsa, const blkptr_t *bp, uint64_t object,
    dnode_phys_t *dnp)
{
	struct drr_object *drro = &(dsa->dsa_drr->drr_u.drr_object);
	int bonuslen;

	if (object < dsa->dsa_resume_object) {
		/*
		 ...
		 */
		ASSERT3U(dsa->dsa_resume_object - object, <,
		    1 << (DNODE_BLOCK_SHIFT - DNODE_SHIFT));
		return (0);
	}

	if (dnp == NULL || dnp->dn_type == DMU_OT_NONE)
		return (dump_freeobjects(dsa, object, 1));
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_OBJECT;
	...
	drro->drr_toguid = dsa->dsa_toguid;

	if (!(dsa->dsa_featureflags & DMU_BACKUP_FEATURE_LARGE_BLOCKS) &&
	...
	if (dump_record(dsa, DN_BONUS(dnp), bonuslen) != 0)
		return (SET_ERROR(EINTR));

	/* Free anything past the end of the file. */
	if (dump_free(dsa, object, (dnp->dn_maxblkid + 1) *
	...
	if (zfs_send_unmodified_spill_blocks &&
	    ...
	    (BP_GET_BIRTH(DN_SPILL_BLKPTR(dnp)) <= dsa->dsa_fromtxg)) {
		struct send_block_record record;

		memset(&record, 0, sizeof (struct send_block_record));
		record.eos_marker = B_FALSE;
		record.bp = *DN_SPILL_BLKPTR(dnp);
		SET_BOOKMARK(&(record.zb), dmu_objset_id(dsa->dsa_os),
		    object, 0, DMU_SPILL_BLKID);

		if (do_dump(dsa, &record) != 0)
			return (SET_ERROR(EINTR));
	}
...
static int
dump_object_range(dmu_sendarg_t *dsa, const blkptr_t *bp,
    uint64_t firstobj, uint64_t numslots)
{
	struct drr_object_range *drror =
	    &(dsa->dsa_drr->drr_u.drr_object_range);

	/* we only use this record type for raw sends */
	ASSERT(BP_IS_PROTECTED(bp));
	ASSERT(dsa->dsa_featureflags & DMU_BACKUP_FEATURE_RAW);
	ASSERT3U(BP_GET_COMPRESS(bp), ==, ZIO_COMPRESS_OFF);
	...
	memset(dsa->dsa_drr, 0, sizeof (dmu_replay_record_t));
	dsa->dsa_drr->drr_type = DRR_OBJECT_RANGE;
	drror->drr_firstobj = firstobj;
	drror->drr_numslots = numslots;
	drror->drr_toguid = dsa->dsa_toguid;
	if (BP_SHOULD_BYTESWAP(bp))
...
do_dump(dmu_sendarg_t *dsa, struct send_block_record *data)
{
	...
	if (dsa->dsa_os->os_encrypted &&
		...
		if (err == 0) {
                ...
                arc_buf_destroy(abuf, &abuf);
        } else if (send_do_embed(bp, dsa->dsa_featureflags)) {
    ...
    dmu_send_outparams_t *dso, offset_t *off)
{
	...
	if (featureflags &
		...
		if (featureflags & DMU_BACKUP_FEATURE_RAW) {
			...
			err = dsl_crypto_populate_key_nvlist(os,
			    ivset_guid, &keynvl);
			if (err != 0) {
				fnvlist_free(nvl);
				goto out;
	...
	err = bqueue_init(&to_arg.q, zfs_send_queue_ff,
