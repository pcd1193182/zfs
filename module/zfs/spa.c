...
spa_activate(spa_t *spa, nvlist_t *config, spa_mode_t mode, boolean_t creating)
{
	...
	if (spa->spa_uses_shared_log) {
		spa_zil_update_head_t *entry = kmem_zalloc(sizeof (*entry), KM_SLEEP);
...
spa_load_chain_map(spa_t *spa)
{
	...
	spa_zil_chain_map_value_t *szcmv = kmem_alloc(sizeof (*szcmv), KM_SLEEP);
	for (zap_cursor_init(&zc, os, chain_map_zap);
		...
		if (pool_entry == NULL) {
			...
			strlcpy(pool_entry->scmp_name, szcmv->szcmv_pool_name, ZFS_MAX_DATASET_NAME_LEN); // TODO
    ...
    nvlist_t *nvpolicy, nvlist_t **config)
{
	...
	*spapp = NULL;
	...
	if (spa->spa_state == POOL_STATE_UNINITIALIZED) {
		...
		error = spa_activate(spa, spa->spa_config, spa_mode_global, B_FALSE);
		if (error != 0) {
			spa_remove(spa);
			if (locked)
				mutex_exit(&spa_namespace_lock);
...
spa_chain_map_update(spa_t *spa)
{
	...
	for (node = avl_first(t); node;) {
		...
		if (os_entry == NULL) {
			...
			VERIFY0(zap_add_uint64(target_mos, chain_map_zap, keybuf, sizeof (keybuf) / sizeof (uint64_t),
			    sizeof (uint64_t), sizeof (szcmv) / sizeof (uint64_t),
			    (uint64_t *)&szcmv, tx));
		} else {
			os_entry->scmo_chain_head = node->szuh_chain_head;
			szcmv.szcmv_bp = os_entry->scmo_chain_head;
			VERIFY0(zap_update_uint64(target_mos, chain_map_zap, keybuf, sizeof (keybuf) / sizeof (uint64_t),
			    sizeof (uint64_t), sizeof (szcmv) / sizeof (uint64_t),
			    (uint64_t *)&szcmv, tx));
		}
		node = AVL_NEXT(t, node);
	...
	while ((entry = list_remove_head(l)) != NULL) {
		...
		VERIFY0(zap_remove_uint64(target_mos, chain_map_zap, keybuf, 2, tx));
	}
...
int
spa_recycle_clients(spa_t *spa, nvlist_t *clients, boolean_t dryrun, nvlist_t *outnvl)
{
	int err = 0;
	if (!spa_is_shared_log(spa)) {
		return (SET_ERROR(ENOTSUP));
